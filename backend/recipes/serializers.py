...
class EditServingSizeSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        if 'serving_size' in validated_data:
            ...
            if num <= 0:
                raise serializers.ValidationError('Serving size cannot be zero')
...
class UserInteractSerializer(serializers.ModelSerializer):
    class Meta:
        model = InteractedWith
        fields = ['liked', 'faved', 'shoppinglist']
